...
app.use(async (ctx, next) => {
  ...
  statsdClient.increment('status_code', { service: 'coordinator', status: ctx.status })
})
...
app.use(async ctx => {
  ...
  try {
    ...
    if (ctx.settings.config.requestDeadline != -1) {
      gotOptions.headers['X-Timeout-Limit'] = ctx.settings.config.requestDeadline
    }
    ...
    if (ctx.settings.config.retries.limit > 0) {
      gotOptions.retry = { calculateDelay: ({ attemptCount, computedValue }) => 
        {
    ...
    const response = await trackTiming(async () => {
      return got.get('http://orders:3000', gotOptions)}, (latency) => {
      statsdClient.timing('latency-orders', latency, { service: SERVICE_NAME })
    })
