...
app.use(async (ctx, next) => {
  ...
  statsdClient.increment('status_code', { service: SERVICE_NAME, status: ctx.status })
})
...
fs.watchFile(process.env.CONFIG_FILE_PATH, async () => {
  console.log(`[${new Date().toISOString()}] ${process.env.CONFIG_FILE_PATH} file saved`);
  ...
  if (diffValue.length === 0) {
    console.log(`[${new Date().toISOString()}] ${process.env.CONFIG_FILE_PATH} no changes`);
    return
  }
...
app.use(async ctx => {
  try {
    ...
    if (val < ctx.settings.config.percentageOfSlowedDownRequests) {
      await new Promise((resolve) => setTimeout(resolve, ctx.settings.config.baseLatency)) // hang for 1 hours
      // console.log('>>>', val, concurrency)
      await new Promise((resolve) => setTimeout(resolve, ctx.settings.config.slowedDownRequestLatency)) // hang for 1 hours
      ctx.body = 'HELLO'
      return
    }
    const timeoutLimit = Number(ctx.request.headers['x-timeout-limit'] ?? 10_000)

    if (ctx.settings.config.useNewFeature === true){
      /*
       * 15 concurrent requests -> penalty ~= 11 -> latency ~= 1100 ms
       */
      const concurrencyPenalty = ((concurrency ** 2) / 20)
      // console.log(concurrency, concurrencyPenalty, ctx.settings.config.baseLatency*concurrencyPenalty)
      const totalLatency = Math.min(60 * 60 * 1000, ctx.settings.config.baseLatency * concurrencyPenalty)

      await new Promise((resolve, reject) => {
        // console.log(timeoutLimit, totalLatency)
